	...
	BasicViewWeapon {
		...
		ViewSMGSkin(Renderer@ r, AudioDevice@ dev){
			...
			@logoModel = renderer.RegisterModel
				("Models/Logotypes/SMG/Logotype.kv6");
		...
		void WeaponFired(){
			...
			if(!IsMuted){
				Vector3 origin = Vector3(0.4f, -0.4f, 0.5f);
				AudioParam param;
				param.volume = 8.f;
		...
		void AddToScene() {
			...
			rightHand = mat * Vector3(0.f, -6.f, 1.f);
			
			Vector3 leftHand2 = mat * Vector3(3.f, -3.f, 10.f);
			Vector3 leftHand3 = mat * Vector3(1.f, 4.f, -2.f);
			Vector3 leftHand4 = mat * Vector3(1.f, 0.f, -2.f);
			...
			sightMat *= CreateTranslateMatrix(0.04f, -0.f, -5.0f);
			sightMat *= CreateScaleMatrix(0.08f);
			param.matrix = sightMat;
			renderer.AddModel(sightModel1, param); // rear

			sightMat = weapMatrix;
			sightMat *= CreateTranslateMatrix(0.04f, 6.f, -5.3f);
			sightMat *= CreateScaleMatrix(0.10f);
			param.matrix = sightMat;
			renderer.AddModel(sightModel2, param); // rear

			sightMat = weapMatrix;
			sightMat *= CreateTranslateMatrix(1.050f, 5.5f, -2.4f);
			sightMat *= CreateScaleMatrix(0.03f);
			param.matrix = sightMat;
			renderer.AddModel(MWlogotype, param); // rear

			sightMat = weapMatrix;
			sightMat *= CreateTranslateMatrix(1.050f, 3.5f, -2.5f);
			sightMat *= CreateScaleMatrix(0.03f);
			param.matrix = sightMat;
